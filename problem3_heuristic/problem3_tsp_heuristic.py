...
# The order the datasets appear in the menu, written out in full.
# Listing them explicitly means the program never calls a sorting routine,
# not even to arrange menu items, which keeps the "no built-in algorithm
# functions" rule true of every line in the project rather than only the
# algorithm itself.
DATASET_MENU_ORDER = ["1", "2", "3", "4"]
...
def choose_sample_dataset():
    ...
    for key in DATASET_MENU_ORDER:
        description = SAMPLE_DATASETS[key][0]
        print("  %s. %s" % (key, description))
    print()

    choice = read_choice("  Select a dataset: ", DATASET_MENU_ORDER)
...
def run_analysis(description, names, coords):
    ...
    # The best starting city is already known from the table above, so rebuild
    # just that one tour rather than searching every start again, then apply
    # 2-opt to it as well.
    multi_tour = tsp.nearest_neighbour_tour(dist, cheapest_index)
    ...
    results = [
        ...
        ("Best NN over all starts (%s)" % names[cheapest_index], cheapest_cost),
