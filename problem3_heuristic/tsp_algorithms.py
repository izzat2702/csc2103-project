...
def nearest_neighbour_costs_by_start(dist):
    ...
    cities gives noticeably different tours depending only on where we begin.
    """
...
def best_nearest_neighbour_tour(dist):
    """
    Run NN from every starting city and keep the cheapest tour found.
    Returns (tour, cost, start_city_index).

    Extra feature: plain NN depends heavily on where it starts, so trying all
    n starting points and keeping the best removes that weakness. It multiplies
    the runtime by n, giving O(n^3), which is still far cheaper than exhaustive
    search.
    """
    best_tour = None
    best_cost = float('inf')
